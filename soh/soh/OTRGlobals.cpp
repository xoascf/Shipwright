        ...
        if (Extractor::ShowYesNoBox("Archivo OTR antiguo encontrado", msgBuf) == IDYES) {
            ...
            if (!std::filesystem::exists(installPath + "/assets/extractor")) {
                Extractor::ShowErrorBox(
                    "No se han encontrado los archivos del extractor",
...
extern "C" void InitOTR() {
    ...
    if (tfile == NULL || error) {
        Extractor::ShowErrorBox(
            "Error", "SoH no tiene los permisos de archivo adecuados. Muévalo a una carpeta que sí los tenga y ejecútelo de nuevo.");
    ...
    if (!std::filesystem::exists(Ship::Context::LocateFileAcrossAppDirs("oot-mq.otr", appShortName)) &&
        ...
        if (Extractor::ShowYesNoBox("Extracción completada", "ROM extraída con éxito, ¿desea extraer otra ROM?") == IDYES) {
            ...
            if (!extract.Run(Ship::Context::GetAppDirectoryPath(appShortName),
                ...
                Extractor::ShowErrorBox(
                    "Error",
                    "Se ha producido un error, es posible que se haya creado un archivo OTR en otro paso.\n\nContinuando...");
            } else {
                extract.CallZapd(installPath, Ship::Context::GetAppDirectoryPath(appShortName));
            }
        }
